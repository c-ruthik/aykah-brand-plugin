...
The photographer runs a 24-item quality verification checklist before returning — covers texture fidelity, camera specs, lighting direction, grounding contact shadows, composition technique, real-world scale cues (architectural cues like baseboards / ceiling height / doorways), photorealism cues (film grain, asymmetric placement, light falloff), **EXACTLY ONE wall art piece per scene (v10 rule — never multiple on the same wall; matched 3-piece gallery counted as one unit is exempt)**, rug check (vintage faded oriental / berber / jute allowed; no sisal / flatweave / round / bold geometric), fruit-bowl / plant / vase ban, and studio-mode override (no walls / floors / windows / props if studio).
